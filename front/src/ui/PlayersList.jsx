...
const PlayersList = ({ players }) => {
  ...
  return (
    <div>
      ...
      {groupedPlayers &&
        Object.keys(groupedPlayers).map((list) => {
          return (
            <>
              ...
              <ul className="row-gap-1 flexbox-container ml-10 flex-gap">
                {groupedPlayers[list].map((x) => {
                  return (
                    <li className="row-gap-1 player-card flexbox-container-cols ">
                      {x.number ? (
                        <span className="text-white">#{x.number}</span>
                      ) : (
                        <span className="text-white">No Number</span>
                      )}
                      <img
                        className="border-circle border-white"
                        src={x.photo}
                      ></img>
