...
const Squad = ({ teamid }) => {
    ...
    const fetchData = async () => {
      ...
      if (response.status == 200) {
        if (import.meta.env.DEV) {
          set_players(response.data);
          base_players = response.data;
        } else {
  ...
  useEffect(() => {
    if (position_filter_value) {
      if (position_filter_value !== "All")
        set_players(
          base_players?.filter((x) => x.position === position_filter_value)
        );
  ...
  return (
    <div className="flex flex-cols justify-center relative-10">
